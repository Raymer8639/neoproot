...
// ==================== Bionic 专属快速字符串函数（修复警告版） ====================
static inline size_t __strlen(const char *s) {
    register const char *a = s;
    while (*s) ++s;
    return s - a;
}
...
static inline char* __strncat(char *dst, const char *src, size_t n) {
    char *d = dst;
    while (*d) ++d;
    while (*src && n--) {
        *d++ = *src++;
...
static inline char* __strstr(const char *haystack, const char *needle) {
...
#define STRCMP(a, b) __strcmp(a, b)
#define STRNCMP(a, b, n) __strncmp(a, b, n)
#define STRCPY(a, b) __strcpy(a, b)
#define STRNCPY(a, b, n) strncpy(a, b, n)
#define STRNCAT(a, b, n) __strncat(a, b, n)
#define STRLEN(a) __strlen(a)
#define STRCHR(a, c) __strchr(a, c)
#define STRSTR(a, b) __strstr(a, b)
// ==================== 宏定义结束 ====================
...
static void print_execve_help(const Tracee *tracee, const char *argv0, int status)
{
    ...
    if (status == -ENOENT && SAFE_GETENV("LD_PRELOAD") != NULL && STRSTR(SAFE_GETENV("LD_PRELOAD"), "libtermux-exec.so") != NULL) {
...
const char *expand_front_variable(TALLOC_CTX *ctx, const char *s)
{
    const char *su;
    char *ex;
    ptrdiff_t sz;
