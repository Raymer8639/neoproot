		...
		char **rpaths, char **runpaths)
{
	...
	dyn_off = PROGRAM_FIELD(*elf_header, dyn, offset);
	dyn_sz  = PROGRAM_FIELD(*elf_header, dyn, filesz);
	ent_sz  = IS_CLASS32(*elf_header) ? sizeof(DynamicEntry32) : sizeof(DynamicEntry64);
	...
	for (i = 0; i < dyn_sz / ent_sz; i++) {
		...
		tag = DYNAMIC_FIELD(*elf_header, e, tag);
		val = DYNAMIC_FIELD(*elf_header, e, val);
	...
	if (straddr == ULLONG_MAX) return 0;
	...
	stroff = PROGRAM_FIELD(*elf_header, dyn, offset)
	       + (straddr - PROGRAM_FIELD(*elf_header, dyn, vaddr));
...
int rebuild_host_ldso_paths(Tracee *tracee, const char host_path[PATH_MAX], ArrayOfXPointers *envp)
{
	...
	if (IS_CLASS32(elf_header))
		add_host_ldso_paths(host_ldso_paths,
			"/lib32:/usr/lib32:/lib:/usr/lib:/system/lib");
	else
		add_host_ldso_paths(host_ldso_paths,
