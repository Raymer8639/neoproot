...
// 真正修复：标记 constructor + used，让 GCC 知道函数会被自动调用
static void __attribute__((used)) loader_ctx_cleanup(void)
{
    if (g_loader_ctx) {
...
static void __attribute__((used)) loader_ctx_init(void)
{
    if (!g_loader_ctx) {
...
static int map_segment(LoadInfo *info, const ProgramHeader *ph)
{
    ...
    // 纯64位ELF，直接使用class64成员，移除32位兼容判断
...
static void compute_load_addresses(Tracee *tr)
{
    ...
    // 彻底移除32位加载器条件编译与32位ELF判断，仅保留64位逻辑
