...
{
	size_t shm_index __attribute__((unused)); // 修复：添加未使用标记
...
{
	size_t shm_index __attribute__((unused));
	struct SysVIpcSharedMem *shm;
	LOOKUP_IPC_OBJECT(shm_index, shm, config->ipc_namespace->shms)
...
void sysvipc_shm_helper_main() {
	...
	for (int i = 0;; i++) {
		...
		// 修复：替换废弃的mktemp，用snprintf生成安全的临时路径
		snprintf(path, PATH_MAX, "/tmp/prootshm-XXXXXX");
