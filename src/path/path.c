...
static inline int fast_strncmp(const char *a, const char *b, size_t n) {
    if (n == 0) return 0;
...
//=============================================================================
// LRU 路径缓存（厂商风格：小容量、高频命中、优先系统路径）
//=============================================================================
#define PATH_CACHE_COUNT 32
#define PATH_CACHE_MASK (PATH_CACHE_COUNT - 1)
...
static PathCacheEntry path_cache[PATH_CACHE_COUNT];
static uint32_t path_cache_clock;

// 优先缓存的系统路径前缀（厂商核心优化：只缓存高频路径）
static const char * const system_paths[] = {
    "/bin", "/sbin", "/usr", "/etc", "/lib", "/lib64",
    "/proc", "/sys", "/dev", "/tmp", "/run", "/var",
    "/system", "/vendor", "/product", "/odm", NULL
};
...
static bool is_system_path(const char *path) {
    ...
    for (int i = 0; system_paths[i]; i++) {
        const char *pre = system_paths[i];
        size_t l = strlen(pre);
        if (fast_strncmp(path, pre, l) == 0 && (path[l] == '/' || path[l] == 0))
            return true;
    }
    return false;
}
...
// 查找缓存
static int path_cache_lookup(const char *guest, char *host_out) {
    for (int i = 0; i < PATH_CACHE_COUNT; i++) {
        PathCacheEntry *e = &path_cache[i];
        if (e->used && fast_strcmp(e->guest, guest) == 0) {
            strcpy(host_out, e->host);
            e->used = ++path_cache_clock;
            return 1;
        }
    }
    return 0;
...
// 插入缓存（只缓存系统路径 & 短路径，避免爆炸）
static void path_cache_insert(const char *guest, const char *host) {
    if (!is_system_path(guest)) return; // 只缓存系统路径，这是关键

    size_t glen = strlen(guest);
    size_t hlen = strlen(host);
    if (glen >= PATH_MAX || hlen >= PATH_MAX) return;

    // LRU 淘汰：找最久未使用
    PathCacheEntry *victim = &path_cache[0];
    for (int i = 1; i < PATH_CACHE_COUNT; i++) {
        if (path_cache[i].used < victim->used)
            victim = &path_cache[i];
    }

    strcpy(victim->guest, guest);
    strcpy(victim->host, host);
    victim->used = ++path_cache_clock;
}

/*============================================================================*\
                     扩展通知：保留原版遍历，仅消除无用遍历开销
\*============================================================================*/

// 直接复用原版 notify_extensions，不引入新符号，避免链接错误
#undef notify_extensions
#define notify_extensions  original_notify_extensions

static inline int original_notify_extensions(Tracee *tracee, ExtensionEvent event, intptr_t d1, intptr_t d2)
{
    Extension *ext;

    if (!tracee || !tracee->extensions)
        return 0;

    LIST_FOREACH(ext, tracee->extensions, link) {
        int st = ext->callback(ext, event, d1, d2);
        if (st != 0)
            return st;
    }
    return 0;
                   ...
                   const char *user_path, bool deref_final)
{
    ...
    if (user_path[0] == PATH_SEP) {
        ...
        char base[PATH_MAX];
        if (dir_fd != AT_FDCWD) {
            status = readlink_proc_pid_fd(tracee->pid, dir_fd, base);
            if (status < 0) return status;
            status = detranslate_path(tracee, base, NULL);
            if (status < 0) return status;
        } else {
            status = getcwd2(tracee, base);
            if (status < 0) return status;
        }
        if (join_paths(2, guest_path, base, user_path) < 0)
            return -ENAMETOOLONG;
    }

    // ===================== LRU 缓存命中：直接返回 =====================
    if (path_cache_lookup(guest_path, result)) {
        return 0;
    }
    ...
    if (status < 0) return status;
    if (status > 0) goto skip_guest;

    char canonical[PATH_MAX];
    strcpy(canonical, "/");
    status = canonicalize(tracee, guest_path, deref_final, canonical, 0);
    if (status < 0) return status;

    status = substitute_binding(tracee, GUEST, canonical);
    if (status < 0) return status;
...
skip_guest:
    // ===================== 缓存系统路径（厂商策略） =====================
    path_cache_insert(guest_path, result);
...
int detranslate_path(Tracee *tracee, char path[PATH_MAX], const char t_referrer[PATH_MAX])
{
    ...
    if (t_referrer != NULL) {
        ...
        // 优化：字面量"/proc"直接传编译期长度5，省去运行时strlen
        comparison = compare_paths2("/proc", 5, t_referrer, strlen(t_referrer));
...
// 优化：增加always_inline，让编译器内联消除调用开销
...
Comparison compare_paths2(const char *path1, size_t length1, const char *path2, size_t length2)
{
    ...
    const bool is_prefix = (fast_strncmp(path1, path2, length_min) == 0);
    if (!is_prefix)
        return PATHS_ARE_NOT_COMPARABLE;
