...
static void set_placeholder_destructor(TALLOC_CTX *parent_ctx, const char *path) {
    // 扁平：直接挂在 parent_ctx 上，不再嵌套
    char *copy = talloc_strdup(parent_ctx, path);
    if (copy)
