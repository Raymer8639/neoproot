...
// 对象池：重用 chained_syscall 对象
static struct chained_syscalls g_syscall_pool = STAILQ_HEAD_INITIALIZER(g_syscall_pool);

static struct chained_syscall *get_from_pool(void) {
	if (!STAILQ_EMPTY(&g_syscall_pool)) {
...
static void return_to_pool(struct chained_syscall *syscall) {
	if (syscall != NULL) {
...
void chain_next_syscall(Tracee *tracee)
{
	...
	/* Return the object to the pool instead of freeing it */
