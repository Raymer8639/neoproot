...
static const SyscallHandlerEntry syscall_handlers[] = {
    { PR_accept,        [](Tracee* t __attribute__((unused))) -> int { return 0; } },
    { PR_accept4,       [](Tracee* t __attribute__((unused))) -> int { return 0; } },
    ...
    { PR_getpeername,   [](Tracee* t __attribute__((unused))) -> int { return 0; } },
    { PR_getsockname,   [](Tracee* t __attribute__((unused))) -> int { return 0; } },
    ...
    { PR_ioctl,         [](Tracee* t) -> int {
#ifdef __ANDROID__
        reg_val_t val = peek_reg(t, CURRENT, SYSARG_2);
        if (val == (reg_val_t)(TCSETS+2)) poke_reg(t, SYSARG_2, (reg_val_t)TCSETS);
        else if (val == TCGETS2) poke_reg(t, SYSARG_2, TCGETS);
        else if (val == TCSETS2) poke_reg(t, SYSARG_2, TCSETS);
        else if (val == TCSETSW2) poke_reg(t, SYSARG_2, TCSETSW);
        else if (val == TCSETSF2) poke_reg(t, SYSARG_2, TCSETSF);
#endif
        return 0;
    }},
    ...
    { PR_memfd_create,  [](Tracee* t) -> int {
        char name[20] = {0};
        int ret = read_string(t, name, peek_reg(t, CURRENT, SYSARG_1), sizeof(name)-1);
        if (ret >= 0) {
            if (!strncmp(name, "JITCode:", 8) || !strcmp(name, "opcache_lock") || !strncmp(name, "lib/apk/exec/", 13))
                return -EACCES;
        }
        return 0;
    }},
    ...
    { PR_socketcall,    [](Tracee* t __attribute__((unused))) -> int { return 0; } },
...
static syscall_handler_t find_syscall_handler(sysnum_t num) {
    const SyscallHandlerEntry* e = (const SyscallHandlerEntry*)bsearch(
...
extern "C" int translate_syscall_enter(Tracee* tracee)
{
    ...
    int status2 = 0;
    sysnum_t num = 0;
    syscall_handler_t handler = nullptr;
    ...
    handler = find_syscall_handler(num);
