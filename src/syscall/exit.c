...
#undef POKE_WORD
	...
	case PR_renameat: {
		...
		// 树扁平：直接挂 tracee，不再挂 tracee->fs
		if (tracee->fs->cwd && strlen(tracee->fs->cwd) >= strlen(old_path)) {
			strcpy(tracee->fs->cwd, old_path);
		} else {
			tmp = talloc_strdup(tracee, old_path);
			if (tmp == NULL) {
				status = -ENOMEM;
				break;
			}
	...
	case PR_statfs64: {
		...
		if (comparison == PATHS_ARE_EQUAL || comparison == PATH1_IS_PREFIX) {
			...
			VERBOSE(tracee, 5, "statfs() not for /dev/shm, not changing statfs() result");
		}
