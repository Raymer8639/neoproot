...
{
    int size;
    const word_t src = peek_reg(tracee, CURRENT, reg); // 常量化，仅读一次
...
{
    const word_t tracee_ptr = alloc_mem(tracee, size); // 常量化，仅分配一次
    if (tracee_ptr == 0)
        return -EFAULT;
...
{
    if (path == NULL) // 空指针防护，避免strlen崩溃
        return set_sysarg_data(tracee, "", 1, reg);
...
void translate_syscall(Tracee *tracee)
{
    ...
    word_t orig_sysnum, current_sysnum; // 提前声明，规避C23标签后声明警告
    bool override_sysnum = false;       // 提前声明，统一作用域
    ...
    override_sysnum = is_enter_stage && tracee->chain.syscalls == NULL;
    const int push_regs_status = push_specific_regs(tracee, override_sysnum);

    /* Handle inability to change syscall number */
    if (push_regs_status < 0 && override_sysnum) {
        orig_sysnum = peek_reg(tracee, ORIGINAL, SYSARG_NUM);
        current_sysnum = peek_reg(tracee, CURRENT, SYSARG_NUM);
        ...
        if (orig_sysnum != current_sysnum) {
            ...
            poke_reg(tracee, SYSARG_1, (word_t)-1);
            poke_reg(tracee, SYSARG_2, (word_t)-1);
            poke_reg(tracee, SYSARG_3, (word_t)-1);
            poke_reg(tracee, SYSARG_4, (word_t)-1);
            poke_reg(tracee, SYSARG_5, (word_t)-1);
            poke_reg(tracee, SYSARG_6, (word_t)-1);
            ...
            const int retry_push_status = push_specific_regs(tracee, false);
            if (retry_push_status != 0) {
